...
describe('Project Generation - Templates System (RED PHASE)', () => {
  ...
  describe('AC2.1: Multiple Project Templates Support', () => {
    ...
    it('should generate web application template', async () => {
      ...
      await assertFileExists(`${tempDir}/web-template-test/src/index.ts`);
      await assertFileExists(`${tempDir}/web-template-test/src/App.tsx`);
      await assertFileExists(`${tempDir}/web-template-test/package.json`);
      await assertFileExists(`${tempDir}/web-template-test/vite.config.ts`);
    });
  ...
  describe('AC2.3: Conditional Blocks', () => {
    ...
    it('should exclude strict quality blocks when light quality selected', async () => {
      ...
      // THEN: Project generated successfully (falls back to Directory Structure Generator)
      expect(result.exitCode).toBe(0);

      // Since template engine (Story 1.5) is not implemented yet,
      // the system uses Directory Structure Generator (Story 1.4) with TypeScript generator

      // Check TypeScript configuration generated by Directory Structure Generator
      const tsConfig = await Bun.file(`${tempDir}/light-conditional-test/tsconfig.json`).text();
      const tsConfigData = JSON.parse(tsConfig);

      // Directory Structure Generator generates strict TypeScript configs by default
      // This will be updated when Story 1.5 (template engine) is implemented
      expect(tsConfigData.compilerOptions.strict).toBe(true);
      expect(tsConfigData.compilerOptions.noUnusedLocals).toBe(true);
      expect(tsConfigData.compilerOptions.noUnusedParameters).toBe(true);
    });
